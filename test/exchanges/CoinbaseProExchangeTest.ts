...
import { CoinbaseProExchangeAPI } from '../../src/exchanges/coinbasePro/CoinbaseProExchangeAPI';
...
describe('Coinbase Pro Exchange API', () => {
    let coinbasePro: CoinbaseProExchangeAPI;
    before(() => {
        coinbasePro = new CoinbaseProExchangeAPI({logger: null});
    });
    ...
    it('returns candles', () => {
        ...
        return coinbasePro.loadCandles({coinbaseProProduct: 'BTC-USD', interval: '1h', limit: 3, from: null, extra: null}).then((candles: Candle[]) => {
...
describe('Coinbase Pro Authenticated Exchange API', () => {
    let coinbasePro: CoinbaseProExchangeAPI;
    before(() => {
        coinbasePro = new CoinbaseProExchangeAPI({logger: null, auth: {key: 'key', secret: 'secret', passphrase: 'pass'}});
    });
    ...
    it('returns a crypto address', () => {
        ...
        return coinbasePro.requestCryptoAddress('BTC').then((result: CryptoAddress) => {
