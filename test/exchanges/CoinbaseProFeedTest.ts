...
import { COINBASE_PRO_WS_FEED, CoinbaseProFeed } from '../../src/exchanges/coinbasePro/CoinbaseProFeed';
import { NullLogger } from '../../src/utils/Logger';
import { EventEmitter } from 'events';
import { Big } from '../../src/lib/types';
import { CoinbaseProL2UpdateMessage, CoinbaseProMatchMessage, CoinbaseProMessage, CoinbaseProSnapshotMessage } from '../../src/exchanges/coinbasePro/CoinbaseProMessages';
...
describe('Coinbase Pro Message feed', () => {
    const messages: CoinbaseProMessage[] = [
        {
            type: 'l2update',
            product_id: 'BTC-USD',
            changes: [['buy', '100.0', '10.0']]
        } as CoinbaseProL2UpdateMessage,
        {
            type: 'l2update',
            product_id: 'BTC-USD',
    ...
    let coinbaseProFeed: CoinbaseProFeed;

    before(() => {
        simple.mock(hooks, 'WebSocket').callFn(() => mockSocket);
        coinbaseProFeed = new CoinbaseProFeed({
            logger: null,
            auth: null,
            apiUrl: '',
            wsUrl: '',
