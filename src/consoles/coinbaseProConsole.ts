...
import { AuthCallOptions,
         CoinbaseProAuthConfig,
         CoinbaseProConfig } from '../exchanges/coinbasePro/CoinbaseProInterfaces';
...
const auth: CoinbaseProAuthConfig = {
...
const coinbaseProApi = new CoinbaseProExchangeAPI(coinbaseProConfig);
...
function makeGenericRequest(method: string, url: string, body: string): Promise<any> {
    ...
    return coinbaseProApi.authCall(method, url, opts).then((res: Response) => {
        return res.body;
    }).catch((res: any) => {
...
if (program.ticker) {
    ...
    coinbaseProApi.loadTicker(program.product).then((ticker: Ticker) => {
...
if (program.transfer) {
    ...
    coinbaseProApi.requestTransfer(params).then((result: TransferResult) => {
...
if (program.newMarketOrder && hasAuth()) {
    ...
    coinbaseProApi.placeOrder(params).then((result: LiveOrder) => {
        console.log(printSeparator());
        console.log(msg);
        console.log(result);
    }).catch(logError);
...
if (program.balances && hasAuth()) {
    ...
    coinbaseProApi.loadBalances().then((balances: Balances) => {
        console.log(printSeparator());
...
if (program.orders && hasAuth()) {
    ...
    console.log(printSeparator());
    coinbaseProApi.loadAllOrders(program.product).then((orders: LiveOrder[]) => {
...
if (program.cancelAllOrders && hasAuth()) {
    ...
    console.log(printSeparator());
    coinbaseProApi.cancelAllOrders(program.product).then((orders: string[]) => {
